...
export function validateUrl(url) {
    ...
    if (!s) return { ok: false, reason: N_('URL is empty') };
    let uri;
    try {
        uri = GLib.Uri.parse(s, GLib.UriFlags.NONE);
    } catch (_) {
        return { ok: false, reason: N_('Malformed URL') };
    }
    const scheme = (uri.get_scheme() || '').toLowerCase();
    if (scheme !== 'http' && scheme !== 'https')
        return { ok: false, reason: N_('Only http(s) URLs are supported') };
    if (!uri.get_host())
        return { ok: false, reason: N_('URL has no host') };
    return { ok: true };
}
